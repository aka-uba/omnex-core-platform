...
import { WhatsAppTemplateEditorSkeleton } from './WhatsAppTemplateEditorSkeleton';
...
export function WhatsAppTemplateEditorClient({
  ...
  const { data: template, isLoading } = useQuery<WhatsAppTemplate>({
    queryKey: ['whatsappTemplate', templateId],
    queryFn: async () => {
  ...
  if (isEdit && isLoading) {
    return <WhatsAppTemplateEditorSkeleton />;
  }

  return (
    <Container size="xl" pt="xl">
      <CentralPageHeader
        title={isEdit ? t('editWhatsAppTemplate') : t('createWhatsAppTemplate')}
        description={t('whatsappTemplateDescription')}
        ...
        breadcrumbs={[
          ...
          { label: isEdit ? 'editWhatsAppTemplate' : 'createWhatsAppTemplate', namespace: 'modules/notification-templates' },
        ]}
        actions={[
          {
            label: t('cancel'),
