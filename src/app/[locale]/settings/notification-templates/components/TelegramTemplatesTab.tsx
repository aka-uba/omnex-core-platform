...
export function TelegramTemplatesTab({ locale }: { locale: string }) {
  ...
  const { data, isLoading, error, refetch } = useQuery<TelegramTemplate[]>({
    queryKey: ['notificationTemplates', 'telegram', categoryFilter, typeFilter],
    queryFn: async () => {
      const params = new URLSearchParams();
      params.append('channel', 'telegram');
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((template) =>
      ...
      template.socialContent?.toLowerCase().includes(searchLower)
    );
  }, [data, search]);
  ...
  const columns: DataTableColumn[] = [
    {
      ...
      label: t('table.name'),
      sortable: true,
      render: (value, row: TelegramTemplate) => (
        <Group gap="xs">
      ...
      render: (value, row: TelegramTemplate) => (
        <Group gap="xs" justify="flex-end">
          <ActionIcon
            variant="subtle"
            color="blue"
            onClick={() => router.push(`/${locale}/settings/notification-templates/telegram/${row.id}/edit`)}
    ...
    <Stack gap="md">
      <Group>
        ...
        <Button
          ...
          onClick={() => router.push(`/${locale}/settings/notification-templates/telegram/create`)}
          variant="filled"
        >
          {t('create')}
        </Button>
