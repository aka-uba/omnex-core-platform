...
export function EmailTemplatesTab({ locale }: { locale: string }) {
  ...
  const { data, isLoading, error, refetch } = useQuery<EmailTemplate[]>({
    queryKey: ['notificationTemplates', 'email', categoryFilter, typeFilter],
    queryFn: async () => {
  ...
  const columns: DataTableColumn[] = [
    {
      ...
      label: t('table.name'),
      sortable: true,
      render: (value, row: EmailTemplate) => (
        <Group gap="xs">
    ...
    },
    {
      ...
      render: (value) => {
        const styleLabels: Record<string, string> = {
          corporate: t('styles.corporate'),
          visionary: t('styles.visionary'),
          elegant: t('styles.elegant'),
    ...
    <Stack gap="md">
      <Group>
        ...
        <Button
          ...
          onClick={() => router.push(`/${locale}/settings/notification-templates/email/create`)}
          variant="filled"
        >
          {t('create')}
        </Button>
