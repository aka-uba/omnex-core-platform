...
export function SMSTemplatesTab({ locale }: { locale: string }) {
  ...
  const { data, isLoading, error, refetch } = useQuery<SMSTemplate[]>({
    queryKey: ['notificationTemplates', 'sms', categoryFilter, typeFilter],
    queryFn: async () => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((template) =>
      ...
      template.smsContent?.toLowerCase().includes(searchLower)
    );
  }, [data, search]);
      ...
      render: (value, row: SMSTemplate) => (
        <Group gap="xs" justify="flex-end">
          <ActionIcon
            variant="subtle"
            color="blue"
            onClick={() => router.push(`/${locale}/settings/notification-templates/sms/${row.id}/edit`)}
    ...
    <Stack gap="md">
      <Group>
        ...
        <Button
          ...
          onClick={() => router.push(`/${locale}/settings/notification-templates/sms/create`)}
          variant="filled"
        >
          {t('create')}
        </Button>
