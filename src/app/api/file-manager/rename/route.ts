...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!itemPath || !newName) {
            return NextResponse.json(
                { success: false, error: 'Path and new name are required' },
                { status: 400 }
            );
        }
        ...
        if (itemPath.includes('..') || newName.includes('..') || newName.includes('/') || newName.includes('\\')) {
            return NextResponse.json(
                { success: false, error: 'Invalid path or name' },
                { status: 400 }
            );
        }
