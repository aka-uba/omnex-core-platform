...
# Omnex Core Platform - Dev Server (WSL2/Linux)
# Development server'ı başlatır ve gerekli kontrolleri yapar
...
if [ ! -d "node_modules/.prisma/tenant-client" ]; then
    ...
    run_npx prisma generate --schema=prisma/tenant.schema.prisma
    echo ""
fi
...
if [ $? -ne 0 ]; then
    ...
    run_npx prisma migrate dev --schema=prisma/core.schema.prisma --name init
    echo ""
    echo "[BILGI] Core veritabani seed ediliyor..."
    run_npm run db:seed:core
    echo ""
fi
